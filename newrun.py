...
def setData(data):
	...
	current_position_y = data.pose.pose.position.y
	current_position_z = data.pose.pose.position.z
...
#	print current_position_x, position_x
...
def img_cb_left(data):
	...
	if(first):
		first = False
		last_box = [boxes, scores, classes]
		position_x = current_position_x
		position_y = current_position_y
		position_z = current_position_z
		rotation_matrix = getRotationMatrix(current_x, current_y, current_z, current_w)
		return
...
#			print R
			...
			posx1 = dataleft[0][0][0]*height
			posy1 = dataleft[0][0][1]*width

			posx2 = last_box[0][0][0]*height
			posy2 = last_box[0][0][1]*width
...
#			print A, B, X
			...
			print "Estimated Distance", (X[0]**2 + X[1]**2 + X[2]**2)**0.5, scores[0]
			cv2.imwrite("IMAGE_"+str(count_left)+str(d)+".jpg", encoded_	img)
			position_x = current_position_x
			position_y = current_position_y
			position_z = current_position_z
